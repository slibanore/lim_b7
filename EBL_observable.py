...
def plot_dNgdz():
    ...
    Nsp =  np.zeros(len(z_spherex))
    Ndesi =  np.zeros(len(z_desi))

    for i in range(len(z_sdss)):
        N[i] = dNgdz(z_sdss[i],'SDSS')*delta_zi('SDSS')
    #for i in range(len(z_spherex)):
    #    Nsp[i] = dNgdz(z_spherex[i],'SPHEREx')*delta_zi('SPHEREx')
    for i in range(len(z_desi)):
        Ndesi[i] = dNgdz(z_desi[i],'DESI')*delta_zi('DESI') #if z_desi[i] < 1.6 else dNgdz(z_desi[i],'DESI')*7*delta_zi('DESI')
        # if instead you multiply this by 0.1 and divide by 14e3 deg^2, you obtain fig 13 in 2306.06307, where we have BLG for z < 0.5, LRG up to 1.1, ELG to 1.6, quasar above 

    print(np.trapz(N/delta_zi('SDSS'),z_sdss))
    #print(np.trapz(Nsp,z_spherex))
   ...
   # Create a figure and axis
    ...
    plt.plot(z_desi[:len(Ndesi_smoothed)], Ndesi_smoothed,color=color_ULTRASAT,label=r'$\rm DESI$',)
