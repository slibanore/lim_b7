...
from .modelling.overall_setting import * 
...
astro_Lcut_fid = 50.*u.Lsun

SFR_file_fid = os.getcwd() + '/LIM_b7/modelling/SFR_tables/sfr_release.dat'

astro_par_fid = dict(alpha=astro_alpha_fid,beta=astro_beta_fid, dMF=astro_dMF_fid,sig_SFR=astro_sig_SFR_fid, dndL_Lcut=astro_Lcut_fid, SFR_file=SFR_file_fid)


Mmin_fid = 1e9*u.Msun
Mmax_fid = 1e15*u.Msun
...
obs_params_highz = lambda z, which: \
		dict(nu = rest_nu*u.GHz,
			...
			tobs =  10000*u.hr if which == 'wide' else 7000.*u.hr,
			Omega_field =  0.5*u.deg**2 if which == 'deep' else 540*u.deg**2 if which == 'wide' else 4*u.deg**2,
...
obs_params_lowz = lambda z, which: \
		dict(nu = rest_nu*u.GHz,
			...
			tobs = 10000*u.hr if which == 'wide' else 5000.*u.hr,
			Omega_field = 0.5*u.deg**2 if which == 'deep' else 540*u.deg**2 if which == 'wide' else 4.*u.deg**2,
