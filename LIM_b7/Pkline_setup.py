...
from .Pkline_computation import *
...
# create lim objects
def lim_Pkline(params=default_par):
    ...
    x2 = get_default_params(Pkline.__init__)
    defpars.update(x2)

    for key in params:
        if key not in defpars:
            raise ValueError(key+" is not a valid input parameter for lim_Pkline. Check your input, please.")
    
    return Pkline(**params)


def import_Pkline(filename, label):

    print(label)
    k, Pk = np.genfromtxt(filename)
