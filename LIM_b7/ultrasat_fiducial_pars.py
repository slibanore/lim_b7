...
# csi_Lyalpha_fid = 1.6
# z0_Lyalpha_fid = 0.87 #3.125 #
# zeta_Lyalpha_fid = 1/4.
# f0_Lyalpha_fid = 0.35 #0.18 #
# SFR0_Lyalpha_fid = 1.29 # Msun yr-1
# t_Lyalpha_fid = 1.25 #0.875 #
...
B_Lyalpha_fid = 0.19
D_Lyalpha_fid = 0.8
astro_par_fid = dict(A_lya=A_Lyalpha_fid,B_lya=B_Lyalpha_fid, D_lya=D_Lyalpha_fid, dndL_Lcut=astro_Lcut_fid, SFR_file=SFR_file_fid)
...
astrocosmo_dict = lambda developer, z: \
	...
	Tmin_VID = 1e-2*u.Jy/u.sr,#u.uK,
	Tmax_VID = 1e3*u.Jy/u.sr, #80.*u.uK,
	do_fast_VID = True,
	do_Jysr = do_Jysr_fid,
	#Ngal_max = 500,
...
obs_params_ULTRASAT = lambda z: dict(\
		nu = rest_nu*u.GHz,
        nuObs = (rest_nu / (1.+ z))*u.GHz, 
        Delta_nu = 2.70e5*u.GHz, 
        dnu = (2.70e5*u.GHz).to(u.MHz),
		mag_limit = 23.,
        nsigma = 5,
...
# ULTRASAT (Libanore, Kovetz (2023) - ULTRASAT white paper)
obs_params_ULTRASAT_CBR = lambda z, Nbin: dict(\
		nu = rest_nu*u.GHz,
        nuObs = (rest_nu / (1.+ z))*u.GHz, 
        Delta_nu = 2.70e5*u.GHz, 
        dnu = (2.70e5*u.GHz/Nbin).to(u.MHz),
		mag_limit = 23.,
        nsigma = 5,
        px_side = 5.45*u.arcsec,
        combined_pixels = 3,
...
# SPHEREx (Libanore, Kovetz (2023) - ULTRASAT white paper)
obs_params_SPHEREx = lambda z: dict(\
		nu = rest_nu*u.GHz,
        nuObs = (rest_nu / (1.+ z))*u.GHz, 
        Delta_nu = 6.80e4*u.GHz, 
        dnu = (rest_nu*u.GHz / (1.+ z) / 41.4).to(u.MHz), 
		#mag_limit = 22.8, # check
        #nsigma = 5, # check
        #px_side = 7.3*u.arcsec, # check 
		input_sigmaN = 981*u.Jy/u.sr, 
        combined_pixels = 2,
        Nfeeds = 1,
        beam_FWHM = 6.2*u.arcsec, 
        tobs = (1*u.yr).to(u.hr), 
        Omega_field = 200*u.deg**2, # full sky
        Nfield = 1)  
